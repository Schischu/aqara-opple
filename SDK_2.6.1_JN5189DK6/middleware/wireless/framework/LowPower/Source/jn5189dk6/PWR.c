...
* Return: -
*---------------------------------------------------------------------------*/
void PWR_ResetTotalSleepDuration(uint32_t initial_count)
{
#if (RTOS_TICKLESS)
    PWR_DBG_LOG("");
    OSA_InterruptDisable();
#ifdef USE_WTIMER
    mLpmTotalSleepDuration = PWR_Get32kTimestamp(); /* now */
#else
    mLpmTotalSleepDuration = initial_count;
#endif
    OSA_InterruptEnable();
#endif
}
...
uint32_t PWR_GetTotalSleepDurationMs(void)
{
    ...
    time_delta = (currentSleepTime - mLpmTotalSleepDuration);

    if (time_delta < 0) /* Has counter wrapped ? */
    ...
    time = mLpmTotalSleepDuration - RTC_GetWakeupCount(RTC);
#endif
...
#ifdef USE_WTIMER
    mLpmTotalSleepDuration = PWR_Get32kTimestamp();
#else
    mLpmTotalSleepDuration = wakeupTimeMs;
#endif
