                              ...
                              const IMAGE_CERT_T * root_cert, uint32_t end_of_ssbl)
{
    ...
    do
    {
        ...
        const BOOT_BLOCK_T * boot_block_img_entry = GetBootBlockPtr((IMG_HEADER_T *) dest_dir_entry->img_base_addr);
        ...
        if (0 != boot_block_img_entry->compatibility_offset && !external_flash_ota)
        {
            /* Check major and minor version
             * Reject update if major are not equal
             * Otherwise check the minor version and do not allow the downgrade
             */
            if (SOTA_GET_BLOB_MAJOR_VERSION(boot_block_img_entry->version) != SOTA_GET_BLOB_MAJOR_VERSION(boot_block_ota.version)
                    || SOTA_GET_BLOB_MINOR_VERSION(boot_block_ota.version) <= SOTA_GET_BLOB_MINOR_VERSION(boot_block_img_entry->version))
            {
...
//#define WAIT_DEBUG
    ...
    do {
        ...
        end_of_ssbl = 0x00 + 2*((uint32_t)SSBL_SIZE);
